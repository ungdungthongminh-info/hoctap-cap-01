...
const SHOP_ITEMS: ShopItem[] = [
  ...
  { id: 'av_wizard', type: 'avatar', emoji: '🧙', name: 'Phù thủy', description: 'Phù thủy đại tài', xpCost: 500, unlockLevel: 7 },
...
function loadPurchased(): string[] {
  try {
    return JSON.parse(localStorage.getItem(LS_PURCHASED) || '["av_cat","av_dog"]');
  } catch {
    return ['av_cat', 'av_dog'];
  }
}

function loadEquipped(): Record<string, string> {
  try {
    return JSON.parse(localStorage.getItem(LS_EQUIPPED) || '{"avatar":"av_cat"}');
  } catch {
    return { avatar: 'av_cat' };
  }
}
...
export function AvatarShopPage() {
  ...
  const handleBuy = (item: ShopItem) => {
    if (purchased.includes(item.id)) {
      ...
      if (item.type === 'avatar') {
        try {
          localStorage.setItem(STORAGE_KEYS.AVATAR_EMOJI, item.emoji);
        } catch {
          // noop
        }
      }
  ...
  return (
    <div className="subpage-shell subpage-shell--compact fade-in max-w-4xl mx-auto">
      <div className="subpage-hero subpage-hero--shop">
        <div className="subpage-hero__avatar"><MascotCharacter size="sm" /></div>
        <div className="subpage-hero__content">
          <div className="subpage-hero__eyebrow">Đổi thưởng</div>
        ...
        <div className="subpage-hero__meta">
          <span className="subpage-hero__pill">✨ {totalXp} XP</span>
          <span className="subpage-hero__pill">👜 {ownedCount} vật phẩm</span>
        </div>
      </div>

      <div className="shop-balance-card">
        <div className="shop-balance-preview">
      ...
      <div className="shop-grid-premium">
        {filtered.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="shop-item-title" style={{ color: 'var(--color-text)' }}>{item.name}</div>
              <div className="shop-item-desc" style={{ color: 'var(--color-text-light)' }}>{item.description}</div>

              {locked ? (
                <div className="shop-item-status" style={{ color: '#9CA3AF' }}>
                  <Lock size={12} /> Level {item.unlockLevel}
                </div>
              ) : owned ? (
                isEquipped ? (
                  <div className="shop-item-status" style={{ color: 'var(--color-primary)' }}>
                    <Check size={12} /> Đang dùng
                  </div>
                ) : (
                  <div className="shop-item-status" style={{ color: 'var(--color-success)' }}>
                    Nhấn để trang bị
                  </div>
                )
              ) : (
