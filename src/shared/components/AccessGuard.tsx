...
export function FeatureGuard({
  ...
  return (
    <div className="fade-in flex items-center justify-center min-h-[70vh] px-4 py-8">
      <div className="card w-full max-w-2xl p-6 text-center">
        ...
        <h1 className="text-2xl font-bold mt-4" style={{ color: 'var(--color-primary-dark)' }}>{title}</h1>
        <p className="text-sm mt-2" style={{ color: 'var(--color-text-light)' }}>{description}</p>
        <div className="mt-4 p-4 rounded-xl text-left" style={{ background: 'var(--color-surface)' }}>
        ...
        <div className="mt-5 flex flex-wrap gap-3 justify-center">
          <button className="btn-primary" onClick={() => window.open(WEB_TOTAL_PRODUCTS_URL, '_blank', 'noopener,noreferrer')}>
            <Sparkles size={16} /> Mua key trên Web Tổng
          </button>
          <button className="btn-outline" onClick={() => navigate('/pricing#web-total-account')}>
...
export function InternalBuildBanner() {
  ...
  return (
    <div className="mx-4 mt-4 rounded-xl px-4 py-2 text-xs font-bold" style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FCD34D' }}>
