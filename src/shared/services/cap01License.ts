...
export function inferPlanFromProduct(productIdRaw: string): string {
  ...
  if (productId === 'bundle_3_grades_12m') return 'standard_1year_3grade';
...
function mapStoragePlanId(entitlement: Cap01Entitlement): string {
  ...
  const resolvePlan = (p: string) => {
    if (['beta_year_299', 'cap01_beta_year_299', 'one_grade_year_299', 'standard_1year_1grade', 'standard_1year_1grade_month'].includes(p)) return 'standard';
    if (['standard_year', 'standard_month', 'standard_1year_3grade', 'standard_lifetime'].includes(p)) return 'standard';
    if (p.startsWith('premium')) return 'premium';
    return null;
  };

  const resolved = resolvePlan(rawPlan) || resolvePlan(fallbackPlan);
...
export async function verifyCap01License(options: { force?: boolean } = {}): Promise<Cap01LicenseCache | null> {
  ...
  // Check both navigator.onLine and desktop offline flag set by Electron audit flow.
  let isOffline = false;
