...
function extractQuotedTokens(s) {
  const tokens = [];
  const re = /'((?:\\'|[^'])*)'|"((?:\\"|[^"])*)"/g;
  let m;
  while ((m = re.exec(s)) !== null) {
    tokens.push((m[1] ?? m[2] ?? '').replace(/\\'/g, "'").replace(/\\"/g, '"'));
  }
  return tokens;
}
...
for (const f of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const options = extractQuotedTokens(arrMatch[1]);
    if (options.length < 2) continue;

    const afterArray = line.slice(line.indexOf(arrMatch[0]) + arrMatch[0].length);
    const answerTok = extractQuotedTokens(afterArray)[0] ?? null;
    ...
    if (answerTok) {
      const answerCount = normalized.filter((v) => v === norm(answerTok)).length;
      if (answerCount > 1) {
        risky.push({ file: f, line: i + 1, type: 'answer_appears_multiple_times', detail: answerTok, q: line.trim() });
      }
    }
...
fs.writeFileSync('backups/question_scan_risks_v2.json', JSON.stringify(summary, null, 2), 'utf8');
console.log('filesScanned=' + summary.filesScanned);
console.log('risks=' + summary.risks);
console.log('report=backups/question_scan_risks_v2.json');
